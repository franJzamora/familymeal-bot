...
class NotificationScheduler:
    """Scheduler de notificaciones de descongelar"""

    def __init__(self, bot):
        self.bot = bot
        # IMPORTANTE: configura la timezone según tu zona horaria.
        # Si estás en España, usa 'Europe/Madrid'.
        # Alternativamente, añade TZ=Europe/Madrid en las variables de entorno de Railway
        # y usa timezone='UTC' para que el scheduler use la hora del sistema.
        tz = os.getenv("BOT_TIMEZONE", "Europe/Madrid")
    ...
    def start(self):
        ...
        self.scheduler.add_job(
            ...
            CronTrigger(minute=0),  # Ejecutar cada hora en punto (HH:00)
            id='defrost_reminder',
            replace_existing=True,
            misfire_grace_time=300  # Si se pierde, ejecutar hasta 5 min tarde
        )
        self.scheduler.start()
        logger.info("✅ Scheduler de notificaciones ACTIVADO - ejecutará cada hora en punto")
    ...
    async def check_and_send_reminders(self):
        ...
        Busca meal_plans para MAÑANA cuyo defrost_reminder_time == hora actual.
        Envía notificación a todos los miembros de la familia.
        """
        logger.info("🔔 Ejecutando check de recordatorios...")
        try:
            tomorrow = (datetime.now() + timedelta(days=1)).date()

            # Formato HH:00:00 — así lo devuelve Supabase para columnas TIME
            current_hour_str = datetime.now().strftime('%H:00:00')

            logger.info(f"📅 Buscando meal_plans para mañana: {tomorrow}, hora: {current_hour_str}")

            # Buscar planes de mañana con defrost_reminder_time == hora actual
            meal_plans = supabase.table("meal_plans") \
                .select("*, recipes(id, name, needs_defrost)") \
                .eq("date", str(tomorrow)) \
                .eq("defrost_reminder_time", current_hour_str) \
                .execute()

            logger.info(f"📋 Encontrados {len(meal_plans.data)} meal_plans con recordatorio ahora")

            if not meal_plans.data:
                logger.info("✅ Nada que notificar esta hora")
                return

            for plan in meal_plans.data:
                logger.info(f"➡️ Procesando plan id={plan.get('id')}, familia={plan.get('family_id')}, receta={plan.get('recipes', {}).get('name') if plan.get('recipes') else 'N/A'}")
                await self.send_defrost_reminder(plan, tomorrow)

        except Exception as e:
            logger.error(f"❌ Error en check_and_send_reminders: {e}", exc_info=True)

    async def send_defrost_reminder(self, plan, tomorrow):
        """Enviar recordatorio de descongelar a todos los miembros de la familia"""
        try:
            ...
            if not recipe:
                logger.warning(f"⚠️ Plan {plan.get('id')} no tiene receta asociada")
                return
            ...
            ingredients_resp = supabase.table("recipe_ingredients") \
                .select("*") \
                .eq("recipe_id", recipe_id) \
                .execute()

            if not ingredients_resp.data:
                logger.info(f"ℹ️ Receta '{recipe_name}' no tiene ingredientes registrados")
                return

            # Filtrar solo ingredientes que están en el Congelador
            frozen_items = []
            for ing in ingredients_resp.data:
                inv_item = supabase.table("inventory") \
                    .select("*") \
                    .eq("family_id", family_id) \
                    .eq("section", "Congelador") \
                    .ilike("name", ing['ingredient_name']) \
                    .execute()
            ...
            if not frozen_items:
                logger.info(f"ℹ️ Receta '{recipe_name}' no tiene ingredientes en el Congelador, omitiendo")
                return
            ...
            frozen_list = "\n".join([
                f"• {item['name']} ({item['quantity']} ud)"
                for item in frozen_items
            ])

            message = (
                f"🧊 *Recordatorio de descongelar*\n\n"
                f"Para mañana (*{tomorrow_day} {tomorrow_str}*) necesitas sacar del congelador:\n\n"
                f"{frozen_list}\n\n"
            ...
            members_resp = supabase.table("family_members") \
                .select("users(telegram_id, username)") \
                .eq("family_id", family_id) \
                .execute()

            logger.info(f"👨‍👩‍👧 Enviando a {len(members_resp.data)} miembros de familia {family_id}")
            ...
            for member in members_resp.data:
                ...
                if user_info and user_info.get('telegram_id'):
                    ...
                    try:
                        ...
                        logger.info(f"✅ Recordatorio enviado a @{user_info.get('username', telegram_id)}")
                    except Exception as e:
                        logger.error(f"❌ Error enviando a {telegram_id}: {e}")

        except Exception as e:
            logger.error(f"❌ Error en send_defrost_reminder: {e}", exc_info=True)
...
class FamilyMealBot:
    ...
    # ========== TEST NOTIFICACIÓN (DEBUG) ==========

    async def test_notification(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        /test_notificacion — envía una notificación de prueba al usuario actual.
        Útil para verificar que el sistema funciona sin esperar a la hora en punto.
        """
        telegram_id = update.effective_user.id
        username = update.effective_user.username or update.effective_user.first_name
        first_name = update.effective_user.first_name
        ...
        # Buscar cualquier meal_plan con defrost_reminder_time no nulo
        tomorrow = (datetime.now() + timedelta(days=1)).date()

        await update.message.reply_text(
            f"🔍 *Debug: Buscando datos para notificaciones*\n\n"
            f"Fecha de mañana: `{tomorrow}`\n"
            f"Hora actual (formato BD): `{datetime.now().strftime('%H:00:00')}`\n"
            f"Familia: {family['name']}",
            parse_mode='Markdown'
        )

        # Mostrar todos los meal_plans de la familia para mañana
        ...
        if not plans.data:
            await update.message.reply_text(
                f"⚠️ No hay meal_plans para mañana ({tomorrow}) en tu familia.\n\n"
                f"Añade recetas al menú semanal para probar las notificaciones."
            )
            return

        text = f"📋 *Meal plans de mañana ({tomorrow}):*\n\n"
        for p in plans.data:
            recipe_name = p['recipes']['name'] if p.get('recipes') else "Sin receta"
            reminder_time = p.get('defrost_reminder_time', 'NULL')
            text += f"• {p['meal_type']}: {recipe_name}\n  ⏰ Reminder time: `{reminder_time}`\n"

        await update.message.reply_text(text, parse_mode='Markdown')

        # Intentar enviar notificación de prueba con datos reales si hay planes con reminder
        plans_with_reminder = [p for p in plans.data if p.get('defrost_reminder_time')]
        if plans_with_reminder:
            await update.message.reply_text("🚀 Enviando notificación de prueba...")
            scheduler = context.application.bot_data.get('scheduler')
            if scheduler:
                await scheduler.send_defrost_reminder(plans_with_reminder[0], tomorrow)
                await update.message.reply_text("✅ Notificación de prueba enviada. Revisa arriba en el chat.")
            else:
                await update.message.reply_text("❌ El scheduler no está en bot_data. Algo falló al iniciarlo.")
        else:
            await update.message.reply_text(
                "⚠️ Los meal_plans de mañana no tienen `defrost_reminder_time`.\n\n"
                "Asegúrate de que la receta tiene `needs_defrost = TRUE` y una hora configurada."
            )
...
def main():
    ...
    # ── Hooks del ciclo de vida para iniciar/parar el scheduler ──────────────

    async def post_init(application: Application) -> None:
        """Se ejecuta una vez que el bot está listo. Aquí iniciamos el scheduler."""
        scheduler = NotificationScheduler(application.bot)
        application.bot_data['scheduler'] = scheduler
        scheduler.start()

    async def post_shutdown(application: Application) -> None:
        """Se ejecuta al apagar el bot. Paramos el scheduler limpiamente."""
    ...
    # /test_notificacion — debug manual
    application.add_handler(CommandHandler("test_notificacion", bot.test_notification))
    ...
    application.add_handler(CallbackQueryHandler(
        lambda u, c: u.callback_query.edit_message_text("Cancelado"),
        pattern="^cancel_clear$"
    ))
    ...
    logger.info("🤖 FamilyMeal Bot iniciado - con notificaciones scheduler ACTIVO")
